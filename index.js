...
// ------------- ENV DEĞİŞKENLERİ -------------
const TOKEN = process.env.DISCORD_BOT_TOKEN; // !! BURADAN OKUYORUZ
...
client.once('ready', async () => {
    ...
    // Slash komutlarını sadece bu sunucuya yükle
    try {
    ...
    try {
        ...
        if (interaction.isChatInputCommand()) {
            ...
            if (commandName === 'otoban') {
                ...
                return interaction.reply({
                    content: `✅ Oto-ban / etkinlik mesajı ${channel} kanalına gönderildi.`,
                    ephemeral: true,
                });
            }
...
async function updateOtobanEmbed(msg, data) {
    ...
    const embed = new EmbedBuilder()
        .setTitle(data.closed ? '🎟️ OTOBAN / ETKİNLİK (KAPANDI)' : '🎟️ OTOBAN / ETKİNLİK KAYIT')
        .setDescription(data.description)
        .addFields(
            { name: 'Kişi Sınırı', value: `${data.max}`, inline: true },
            {
                name: 'Durum',
                value: data.closed ? 'Kayıtlar kapandı.' : 'Kayıtlar açık. ✅ ile katılabilirsin.',
                inline: true,
            },
            { name: 'Liste', value: participantsText },
        )
...
client.on('messageReactionAdd', async (reaction, user) => {
    try {
        ...
        await updateOtobanEmbed(msg, data);
    } catch (err) {
...
client.on('messageReactionRemove', async (reaction, user) => {
    try {
        ...
        if (!data.closed && data.participants.has(user.id)) {
            ...
            const msg = await reaction.message.fetch().catch(() => null);
            if (msg) await updateOtobanEmbed(msg, data);
        }
    } catch (err) {
